// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'search_state.dart';

// **************************************************************************
// StoreGenerator
// **************************************************************************

// ignore_for_file: non_constant_identifier_names, unnecessary_brace_in_string_interps, unnecessary_lambdas, prefer_expression_function_bodies, lines_longer_than_80_chars, avoid_as, avoid_annotating_with_dynamic, no_leading_underscores_for_local_identifiers

mixin _$SearchState on SearchStateBase, Store {
  late final _$searchProductsAtom =
      Atom(name: 'SearchStateBase.searchProducts', context: context);

  @override
  ObservableList<ProductModel> get searchProducts {
    _$searchProductsAtom.reportRead();
    return super.searchProducts;
  }

  @override
  set searchProducts(ObservableList<ProductModel> value) {
    _$searchProductsAtom.reportWrite(value, super.searchProducts, () {
      super.searchProducts = value;
    });
  }

  @override
  String toString() {
    return '''
searchProducts: ${searchProducts}
    ''';
  }
}
